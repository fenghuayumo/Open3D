...
/// Local parallel loop for exported helpers (keeps mesh_utils free of TBB).
template <typename Int, typename F>
void parallel_for(Int start, Int end, F &&body) {
#ifdef _OPENMP
    // MSVC OpenMP requires a signed integral loop index; keep callers on size_t/int.
    const long long s = static_cast<long long>(start);
   ...
   std::tuple<uint8_t*, float*, size_t> uniform_down_sample_points(
    ...
    void voxel_down_sample_points_to_file(const std::string& filePath,
        ...
        try {
            ...
            option.write_vertex_float = true;
            open3d::io::WritePointCloud(filePath, *sparse_pcd, option);
        } catch (...) {
            std::cout << "error " << std::endl;
        }
    }
